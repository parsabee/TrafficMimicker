...
static int server_udpSend(const UDPServer *server, void *msg, size_t len) {
  ...
  return sendto(data->theSocket, msg, len, 0, (struct sockaddr *)&(data->client),
...
static int server_udpRecv(const UDPServer *server, void *msg, size_t len) {
  ...
  if (!recvfrom(data->theSocket, msg, len, 0, (struct sockaddr *)&addr, &addrlen))
    return 0;
...
static int server_getClientAddr(const UDPServer *server, struct sockaddr_in *addr) {
  Data *data = (Data *)(server->self);
  if (!data->hasClient)
    return 0;
...
UDPServer serverTemplate = {NULL,     server_udpSend, server_udpRecv,
                            server_getClientAddr, server_destroy};
